...
final class MG2LegacyHistoryBridge {

    static let shared = MG2LegacyHistoryBridge()

    private let useCase: HistoryUseCase

    private init() {
        let provider = DefaultNetworkProvider(session: .default)
        let repository = DefaultHistoryRepository(networkProvider: provider)
        self.useCase = DefaultHistoryUseCase(repository: repository)
    }
    ...
    func editJogak(data: EditJogakRequestMainData, jogakId: Int, completion: @escaping (Result<EditJogakResponse, Error>) -> Void) {
        Task {
            do {
                let response = try await useCase.editJogak(data: data, jogakId: jogakId)
