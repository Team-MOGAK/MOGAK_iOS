...
//        $0.clipsToBounds = true
    ...
    private let userName = UILabel().then {
        $0.text = MG2Deps.app.userState.nickName
        $0.textColor = UIColor(hex: "000000")
        $0.font = UIFont.pretendard(.bold, size: 22)
    }
    
    private let job = UILabel().then {
        $0.text = MG2Deps.app.userState.userJob
        $0.textColor = UIColor(hex: "000000")
    ...
    @objc func userNicknameProfileChangeTapped() {
        ...
        (coordinator ?? MG2MyPageCoordinator()).routeToNicknameEdit(from: navigationController)
    }
    
    //MARK: - 직업 변경 탭 선택
    @objc func userJobChangeTapped() {
        (coordinator ?? MG2MyPageCoordinator()).routeToJobEdit(from: navigationController)
    }
        ...
        viewModel.withdraw { result in
            ...
            case .success(let success):
                ...
                let withdrawAlertAction = UIAlertAction(title: "확인", style: .default)  { _ in
                    ...
                    MG2Deps.app.userState.userAccessToken = ""
                    MG2Deps.app.userState.loginState = .logout
