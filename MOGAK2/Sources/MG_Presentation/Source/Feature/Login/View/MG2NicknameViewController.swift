...
class MG2NicknameViewController: UIViewController {
    ...
    private lazy var nicknameTextField : UITextField = {
        ...
        let placeholderText = MG2Deps.app.userState.nickName != nil && MG2Deps.app.userState.nickName != "" ? MG2Deps.app.userState.nickName ?? "닉네임을 입력해주세요." : "닉네임을 입력해주세요."
...
// 네트워크 코드
extension MG2NicknameViewController {
    //MARK: - 닉네임 타당성 검증 요청
        ...
        profileViewModel.validateNickname(nickName) { result in
            ...
            case .success(let message):
                if message == "성공" {
                    ...
                    (self.coordinator ?? MG2LoginCoordinator()).routeToChooseJob(from: self.navigationController)
                } else {
                    self.makeNicknameErrorAlert(errorMessage: message)
                }
            case .failure(let error):
    ...
    func nicknameChange(nickname: String) {
        ...
        LoadingIndicator.showLoading()
        profileViewModel.changeNickname(nickname) { result in
            LoadingIndicator.hideLoading()
            switch result {
    ...
    func profileImageChangeRequest() {
        ...
        profileViewModel.changeProfileImage(profileImage) { result in
            LoadingIndicator.hideLoading()
            switch result {
            case .success(let success):
                print(#fileID, #function, #line, "- success: \(success)")
